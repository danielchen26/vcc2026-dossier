...
const F0 = (0.5 * nZero) / nCtrl;            // psi(0) / nCtrl
const TARGET = meanCpm * 1.25;               // lfc = +log2(1.25) = +0.3219
const LFC = Math.log2(1.25);

/** psi_g(v) / nCtrl —— 平均对照分位数，由嵌入的真实对照 ECDF 求值 */
...
const STRIP_H = 96;
const LOGP_MIN = -32;
const tx = (t: number) => 50 + (t / 3) * (STRIP_W - 50 - 86);
...
const LIVE_STRIP = (() => {
  ...
  for (let i = 0; i <= 240; i++) {
    const t = (i / 240) * 3;
    const { p } = evaluate(t);
    pts.push(`${i === 0 ? "M" : "L"} ${tx(t).toFixed(1)} ${ty(Math.log10(Math.max(p, 1e-40))).toFixed(1)}`);
  }
  return pts.join(" ");
})();
  ...
  return (
    <div className="dial">
      <div className="dial-top">
        <div className="dial-plot">
          <svg viewBox={`0 0 ${W} ${H}`} role="img"
            aria-label={`真实对照 ECDF 与预测细胞分布，当前 t=${t.toFixed(2)}，p=${pTxt}`}>
            {/* null 带 */}
            ...
            <text x={PAD.l - 8} y={PAD.t - 4} textAnchor="end" className="lb">F(v)</text>
            <text x={W - PAD.r} y={sy(0) + 31} textAnchor="end" className="lb">CPM（√ 刻度）</text>

            {/* 真实对照 ECDF */}
            <path d={ECDF_PATH} fill="none" stroke="#1b4f9c" strokeWidth="1.7" />
            <text x={sx(ecdfQ[Math.floor(ecdfQ.length * 0.8)]) + 4}
              y={sy(0.9)} className="lb" fill="#1b4f9c">
              真实对照 ECDF · {nCtrl.toLocaleString()} 细胞
            </text>
            ...
            <text x={sx(TARGET) + 5} y={PAD.t + 10} className="lb" fill="#6b7a3a">
              一阶矩钉在 {TARGET.toFixed(2)} CPM
            </text>

            {/* 预测细胞的质量：v=0 与 v=s 两处 */}
            {[
            ...
            <line x1={PAD.l} y1={sy(0.5 + st.d)} x2={W - PAD.r} y2={sy(0.5 + st.d)}
              stroke="#b23a2b" strokeWidth="2" />
            <circle cx={W - PAD.r} cy={sy(0.5 + st.d)} r="3.5" fill="#b23a2b" />
            <text x={W - PAD.r + 6} y={sy(0.5 + st.d) + 3.5} className="lb" fill="#b23a2b">
              ψ̄ = {(0.5 + st.d).toFixed(4)}
            </text>
          </svg>
        </div>

        <div className="dial-read">
          <div className={sig ? "verdict sig" : "verdict nul"}>{sig ? "判为显著" : "判为不显著"}</div>
          <div className="pinned">
            一阶矩恒为 <b>{TARGET.toFixed(2)}</b> CPM，lfc 恒为 <b>+{LFC.toFixed(4)}</b>。
            拨盘只改组内分布形状。
          </div>
          {[
            ["分散参数 t", t.toFixed(2)],
            ["非零细胞", `${st.k} / ${nPred}`],
            ["每个非零细胞", `${st.s.toFixed(1)} CPM`],
            ["d = ψ̄ − 0.5", (st.d >= 0 ? "+" : "") + st.d.toFixed(4)],
            ["z", (st.z >= 0 ? "+" : "") + st.z.toFixed(2)],
          ].map(([k, v]) => (
          ...
          <div className="rr">
            <span className="k">p (Wilcoxon)</span>
      ...
      <div className="dial-ctl">
        <label htmlFor="tdial">分散参数 t</label>
      ...
      <div className="dial-ctl" style={{ paddingTop: 0, flexDirection: "column", alignItems: "stretch", gap: 6 }}>
        <svg viewBox={`0 0 ${STRIP_W} ${STRIP_H}`} role="img"
          aria-label="p 值随分散参数变化，橄榄点为本机精确实测值">
          <line x1={50} y1={ty(Math.log10(0.05))} x2={STRIP_W - 86} y2={ty(Math.log10(0.05))}
            stroke="#6b7a3a" strokeWidth="1" strokeDasharray="3 3" />
          <text x={STRIP_W - 82} y={ty(Math.log10(0.05)) + 3.5} className="lb" fill="#6b7a3a">α = 0.05</text>
          <line x1={50} y1={ty(0)} x2={STRIP_W - 86} y2={ty(0)} stroke="#c7cdc1" />
          {[0, -10, -20, -30].map((lp) => (
          ...
          {dial.map((r) => (
            <circle key={r.t} cx={tx(r.t)} cy={ty(Math.log10(r.p))} r="3"
              fill="#6b7a3a" stroke="#fbfcfa" strokeWidth="1" />
          ))}
        ...
        <p style={{ fontSize: 12.5, color: "var(--ink-2)", margin: 0, maxWidth: "none" }}>
          蓝线由嵌入的真实对照 ECDF（{gene} · context {context} · 2048 分位点）实时求值；
          <span style={{ color: "var(--stamp)" }}>橄榄点</span>是本机在完整 18,400 个对照细胞上、
          含并列校正的精确实测值。两者重合即说明浏览器里的算术与打分器一致。
        </p>
      </div>
    </div>
  );
...
export const DIAL_GENE = { gene, context, meanCpm, nZero, nCtrl, target: TARGET, lfc: LFC, sigmaTie, dCrit };
export { evaluate as evalDial };
